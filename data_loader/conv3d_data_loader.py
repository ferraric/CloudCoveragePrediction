...
class DataGenerator:
    def __init__(self, config, comet_logger):
        ...
        cutoff_date = pd.Timestamp('2014-03-31') - pd.Timedelta(hours=121)
        ...
        nwp_train = nwp_train.sel(init_time=nwp_train['init_time.season'] == 'DJF')
        nwp_val = nwp.sel(init_time=slice(val_cutoff, cutoff_date))
        nwp_val = nwp_val.sel(init_time=nwp_val['init_time.season'] == 'DJF')
        ...
        val_values_mean_correct_shape = nwp_val.CLCT_mean.values
        val_values_var_correct_shape = nwp_val.CLCT_var.values
        val_labels_correct_shape = nwp_val.labelValue.values
        ...
        self.validation_data = tf.data.Dataset.from_tensor_slices(
            (np.stack([val_values_mean_correct_shape, val_values_var_correct_shape], axis=-1),
