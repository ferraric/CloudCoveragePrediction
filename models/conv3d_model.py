...
class Conv3dModel(Model):
    ...
    def build_model(self):
        self.conv1 = Conv3D(kernel_size = [3,3,3], filters=4,
                            #dilation_rate=[2, 2, 2],
                            strides=[2,2,2], padding="valid", activation="relu",
                            input_shape=(self.config.batch_size, 121, 59, 90, 2))
        self.conv2 = Conv3D(kernel_size = [3,3,3], filters=8,
                            #dilation_rate=[2, 2, 2],
                            strides=[2,2,2], padding="valid", activation="relu")
        self.deconv1 = Conv3DTranspose(kernel_size = [3,3,3], filters=4,
                                       strides=[2,2,2], padding="valid", activation="relu",
                                       output_padding=[1,0,1])
        self.deconv2 = Conv3DTranspose(kernel_size=[3,3,3], filters=2,
                                       strides=[2,2,2], padding="valid", activation="relu",
