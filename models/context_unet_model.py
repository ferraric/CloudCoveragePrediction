...
from tensorflow.keras.layers import InputLayer, Conv3D, MaxPooling3D, Conv3DTranspose, concatenate, Dropout, BatchNormalization, Embedding


class ContextUNetModel(Model):
    def __init__(self, config):
        super(ContextUNetModel, self).__init__()
        self.config = config
        self.build_model()

    def build_model(self):
        self.inputlayer = InputLayer(input_shape=(self.config.batch_size, 121, 59, 90, 2))
        self.month_embedding = Embedding(12, 105, input_length=1)
