...
class Model_mean_var(tf.Module):
    ...
    def build_model(self):
        ...
        self.init_time_embedding = Embedding(2,
                                             emb_size_init_time,
                                             input_length=1)
        self.lead_time_embedding = Embedding(121,
                                             emb_size_lead_time,
                                             input_length=1)
        self.model1 = tf.keras.Sequential()
        self.model1.add(Dense(layer_1, activation="relu",
                              input_shape=(None, )))
        self.model1.add(
            Dense(layer_2, activation="relu", input_shape=(layer_1, )))
        self.model1.add(
            Dense(layer_3, activation="relu", input_shape=(layer_2, )))
        self.model1.add(
            Dense(layer_4, activation="relu", input_shape=(layer_3, )))
        self.model1.add(
            Dense(layer_5, activation="relu", input_shape=(layer_4, )))
        self.model1.add(
            Dense(layer_6, activation="relu", input_shape=(layer_5, )))
        self.model1.add(
            Dense(layer_7, activation="relu", input_shape=(layer_6, )))
        self.model1.add(
            Dense(layer_8, activation="relu", input_shape=(layer_7, )))
        self.model1.add(
            Dense(layer_9, activation="relu", input_shape=(layer_8, )))
        self.model1.add(Dense(layer_10, input_shape=(layer_9, )))
        self.model2 = tf.keras.Sequential()
        self.model2.add(Dense(layer_1, activation="relu",
                              input_shape=(None, )))
        self.model2.add(
            Dense(layer_2, activation="relu", input_shape=(layer_1, )))
        self.model2.add(
    ...
    def __call__(self, x):
        ...
        concat_all = tf.concat([
            m_emb, h_emb, init_time_emb, lat_lon_emb, lead_time_emb,
            tf.reshape(x[:, 0], [np.shape(x)[0], 1]),
            tf.reshape(x[:, 1], [np.shape(x)[0], 1]),
            tf.reshape(x[:, 2], [np.shape(x)[0], 1]),
            tf.reshape(x[:, 3], [np.shape(x)[0], 1]),
            tf.reshape(x[:, 4], [np.shape(x)[0], 1]),
            tf.reshape(x[:, 5], [np.shape(x)[0], 1]),
            tf.reshape(x[:, 6], [np.shape(x)[0], 1])
        ], 1)
