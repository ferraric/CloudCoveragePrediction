...
class Crps21EnsembleLoss(Loss):
    ...
    def call(self, y_true, y_pred):
        ...
        n = tf.dtypes.cast(tf.shape(y_pred)[0], tf.float64)
        d = tf.dtypes.cast(tf.shape(y_pred)[1], tf.float64)
        y_true = tf.reshape(y_true, [n, 1])
        diffs = tf.math.reduce_sum(tf.math.abs(
            tf.math.subtract(y_pred, tf.broadcast_to(y_true, (n, d)))),
                                   axis=1)
        pair_diff_sum = tf.map_fn(
            lambda row: tf.reduce_sum(
                tf.abs(tf.subtract(row, tf.expand_dims(row, 1)))), y_pred)
