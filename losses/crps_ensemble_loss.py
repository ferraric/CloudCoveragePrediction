...
class CrpsEnsembleLoss(Loss):
    ...
    def call(self, y_true, a):
        ...
        stdev = tf.clip_by_value(stdev,
                                 clip_value_min=0.0001,
                                 clip_value_max=np.inf)
        no_quantiles = 21
        quantiles = [(i - 0.5) / no_quantiles
                     for i in range(1, no_quantiles + 1)]
        quantiles = np.broadcast_to(quantiles, (mu.shape[0], no_quantiles))

        y_ensemble_pred = np.transpose(
            norm.ppf(np.transpose(quantiles), loc=mu, scale=stdev))
        y_ensemble_pred = tf.clip_by_value(y_ensemble_pred,
                                           clip_value_min=0,
                                           clip_value_max=100)
