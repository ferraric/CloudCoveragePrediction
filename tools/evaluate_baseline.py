...
labels_dir = os.path.join("/mnt", "ds3lab-scratch", "bhendj", "grids",
                          "CM-SAF", "MeteosatCFC")
no_of_init_times = 200

for i in range(no_of_init_times):
    init_time = pd.Timestamp(init_time_str)
    persisted_labels_file = os.path.join(
        labels_dir, "meteosat.CFC.H_ch05.latitude_longitude_" +
        init_time_str.replace(' ', '') + "*.nc")
    ...
    df.to_pickle(
        os.path.join(
            save_dir,
            "persistence_CRPS_" + init_time_str.replace(' ', '') + ".pkl"))
