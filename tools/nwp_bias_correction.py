...
with open('/mnt/ds3lab-scratch/srhea/code/dslab2019_meteoswisscloud/shared_data/lookup_xy_ll.pkl', 'rb') as handle:
    label_map_dict_xy_ll = pickle.load(handle)
with open('/mnt/ds3lab-scratch/srhea/code/dslab2019_meteoswisscloud/shared_data/yx_to_ll_dict.pkl', 'rb') as handle:
    yx_to_ll_dict = pickle.load(handle)

with open("/mnt/ds3lab-scratch/srhea/code/bias_dict.pkl","rb") as f:
...
labels = xr.open_mfdataset(
    "/mnt/ds3lab-scratch/bhendj/grids/CM-SAF/MeteosatCFC/meteosat.CFC.H_ch05.latitude_longitude_201[8-9]*.nc",
...
while init_time <= end_date:
    ...
    if os.path.exists("/mnt/ds3lab-scratch/srhea/nwp_bias_corrected_clipped/nwp_crps_scores" + init_time.strftime("%Y-%m-%d-%H") + ".pkl"):
    ...
    for idx_t in range(prediction_data.shape[0]):
        ...
        lead_time=eval_time-init_time
        lead_time=int(lead_time.seconds// 3600)
        print(lead_time)
        ...
        keys = [(lat_array[i], lon_array[i],lead_time,month,init_time_asint) for i in range(0, len(lat_array))]
        ...
        #print(bias)
        crps_scores = ps.crps_ensemble(label_values_for_nwp_grid_points, np.clip(predicted_values-np.reshape(np.repeat(bias,21),[np.shape(bias)[0],21]),0,100))
        print(crps_scores)
        ...
        #f = open("/mnt/ds3lab-scratch/srhea/nwp_bias_corrected_clipped/nwp_crps_scores" + init_time.strftime("%Y-%m-%d-%H") + ".pkl", "wb")
        #pickle.dump(errors, f)
        #f.close()
        print("--- %s seconds ---" % (time.time() - start_timer))
    #df = pd.DataFrame(errors).set_index(["lat", "lon","init_time", "time"])
    save_file ="/mnt/ds3lab-scratch/srhea/nwp_bias_corrected_clipped/nwp_crps_scores" + init_time.strftime("%Y-%m-%d-%H") + ".pkl"
    pd.DataFrame(errors).to_pickle(save_file)
