...
while init_time <= end_date:
    ...
    for idx_t in range(prediction_data.shape[0]):
        ...
        pit_hist_ranks = pit_hist(label_values_for_nwp_grid_points,
                                  predicted_values)
        assert (pit_hist_ranks.shape == lat_array.shape
                and pit_hist_ranks.shape == lon_array.shape)
        errors["lat"].extend(lat_array)
        errors["lon"].extend(lon_array)
        errors["init_time"].extend(
            np.repeat(init_time, pit_hist_ranks.shape[0]))
    ...
    pd.DataFrame(errors).to_pickle(
        os.path.join(
            save_path,
            "pit_hist_ranks" + init_time.strftime("%Y-%m-%d-%H") + ".pkl"))
    init_time += time_step
