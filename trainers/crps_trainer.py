...
from losses.crps_21_ensemble_loss_eval import Crps21EnsembleLossEval
class CRPSTrainer(BaseTrain):
    ...
    def setup_metrics(self):
        ...
        self.ensemble_loss = Crps21EnsembleLossEval()
    ...
    def validation_step(self):
        ...
        with self.comet_logger.test():
            for (x_batch, y_batch) in self.data.validation_data_winter:
                predictions = self.model(x_batch)
                loss = self.ensemble_loss(y_batch, predictions)
    ...
    def save_model(self):
        ...
        tf.saved_model.save(self.model, "new_best_model/")
